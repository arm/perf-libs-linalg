...
MATMUL3_KERNEL_JSON_PATH := $(KERNEL_SPECS_JSON_DIR)/matmul3_kernels.json
...
$(KERNEL_SPECS_BUILD_DIR)/interleave_kernels.hpp: $(MATMUL3_KERNEL_JSON_PATH)
$(KERNEL_SPECS_BUILD_DIR)/interleave_kernels.hpp: KERNEL_SPECS_GENERATOR_OPTIONS := --matmul-kernels $(MATMUL3_KERNEL_JSON_PATH)
$(KERNEL_SPECS_BUILD_DIR)/%.hpp: $(KERNEL_SPECS_JSON_DIR)/%.json $(KERNEL_SPECS_SCRIPTS_DIR)/%.py
	mkdir -p $(@D)
	$(PYTHON) $(KERNEL_SPECS_SCRIPTS_DIR)/$*.py $(KERNEL_SPECS_GENERATOR_OPTIONS) $< $@.tmp
	$(FORMAT) $@.tmp || true
	test -s $@.tmp && mv $@.tmp $@
...
$(GENERATE_DIR)/architectures/%/linalg/kernel_specs/interleave_fallback_instantiations_$1.cpp: $(INTERLEAVE_KERNEL_JSON_PATH) $(INTERLEAVE_KERNEL_GENERATOR) $(MATMUL3_KERNEL_JSON_PATH)
	mkdir -p $$(@D)
	$(PYTHON) $(INTERLEAVE_KERNEL_GENERATOR) --matmul-kernels $(MATMUL3_KERNEL_JSON_PATH) --fallback-instantiations $$* $1 $$< $$@.tmp
	$(FORMAT) $$@.tmp || true
	test -s $$@.tmp && mv $$@.tmp $$@
endef
