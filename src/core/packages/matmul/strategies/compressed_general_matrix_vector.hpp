...
#ifndef PERFLIBS_LINALG_MATMUL_STRATEGIES_COMPRESSED_GENERAL_MATRIX_VECTOR_HPP
#define PERFLIBS_LINALG_MATMUL_STRATEGIES_COMPRESSED_GENERAL_MATRIX_VECTOR_HPP
...
public:
	static constexpr std::string_view name() { return "compressed_general_matrix_vector"; }

	template<typename ProblemContext>
	requires spec::has_get_spec<spec::strategy_tag<compressed_general_matrix_vector>, ProblemContext>
	PERFLIBS_LINALG_INLINE
	bool operator() (const ProblemContext& pctx) const {
		if ( !this->can_compute(pctx) ) return false;

		const auto spec = get_spec(spec::strategy_tag<compressed_general_matrix_vector>{}, pctx);
...
#endif // PERFLIBS_LINALG_MATMUL_STRATEGIES_COMPRESSED_GENERAL_MATRIX_VECTOR_HPP
