...
#ifndef PERFLIBS_LINALG_MATMUL_STRATEGIES_SYMM_HEMM_L_REFERENCE_HPP
#define PERFLIBS_LINALG_MATMUL_STRATEGIES_SYMM_HEMM_L_REFERENCE_HPP
...
class matmul3_symm_hemm_l_reference {
	...
	bool impl(const ProblemContext& pctx) const {
		...
		using a_data_type = typename ProblemContext::a_matrix_type::value_type;

		if(pctx.c.cntg_step() == 1) {
			const auto side       = side_to_c(PERFLIBS_LEFT);
			const auto uplo       = uplo_to_c(lower_flip(pctx.a.uplo()));
			...
			const pl_linalg_int_t lda = pctx.a.cntg_step();
			const auto b          = pctx.b.data();
			const pl_linalg_int_t ldb = pctx.b.strd_step();
			const auto beta       = pctx.beta;
			      auto c          = pctx.c.data();
			const pl_linalg_int_t ldc = pctx.c.strd_step();

			if constexpr(is_symmetric_matrix_v<typename ProblemContext::a_matrix_type>) {
				reference::symm<a_data_type>(&side, &uplo, &m, &n, &alpha, a, &lda, b, &ldb, &beta, c, &ldc);
			}
			else {
				reference::hemm<a_data_type>(&side, &uplo, &m, &n, &alpha, a, &lda, b, &ldb, &beta, c, &ldc);
			}
		}
		else {
	...
	bool can_compute_impl(const ProblemContext& pctx) const {
		return false;
		/*
		TODO: this is current failing for HEMV
...
public:
	static constexpr std::string_view name() { return "matmul3_symm_hemm_l_reference"; }
...
#endif //PERFLIBS_LINALG_MATMUL_STRATEGIES_SYMM_HEMM_L_REFERENCE_HPP
