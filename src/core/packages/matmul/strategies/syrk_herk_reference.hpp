...
#ifndef PERFLIBS_LINALG_MATMUL_STRATEGIES_SYRK_HERK_REFERENCE_HPP
#define PERFLIBS_LINALG_MATMUL_STRATEGIES_SYRK_HERK_REFERENCE_HPP
...
class syrk_herk_reference {
	...
	bool impl(const ProblemContext& pctx) const {
		...
		if constexpr(is_symmetric_matrix_v<typename ProblemContext::c_matrix_type>) {
			...
			reference::syrk<c_data_type>(&uplo, &transa, &n, &k, &alpha, a, &lda, &beta, c, &ldc);
		}
		else {
			PERFLIBS_ASSERT(pctx.alpha.imag() == zero<typename ProblemContext::scalar_type::value_type>, "herk_reference narrows complex alpha to real");
			PERFLIBS_ASSERT(pctx.beta.imag()  == zero<typename ProblemContext::scalar_type::value_type>, "herk_reference narrows complex beta to real");

			const auto alpha = pctx.alpha.real();
			const auto beta  = pctx.beta.real();

			reference::herk<c_data_type>(&uplo, &transa, &n, &k, &alpha, a, &lda, &beta, c, &ldc);
		}
	...
	bool can_compute_impl(const ProblemContext& pctx) const {
		return pctx.c.cntg_step() == 1
		  && ( pctx.a.data() == pctx.b.data() )
		  && ( pctx.a.strd_step() == 1 || pctx.a.cntg_step() == 1)
		  && ( pctx.b.strd_step() == 1 || pctx.b.cntg_step() == 1);
	}

public:
	static constexpr std::string_view name() { return "syrk_herk_reference"; }
...
#endif //PERFLIBS_LINALG_MATMUL_STRATEGIES_SYRK_HERK_REFERENCE_HPP
