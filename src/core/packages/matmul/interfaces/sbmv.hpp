...
#ifndef PERFLIBS_LINALG_BLAS_INTERFACES_SBMV_HPP
#define PERFLIBS_LINALG_BLAS_INTERFACES_SBMV_HPP
...
inline constexpr std::string_view sbmv_name = "?SBMV ";

template<> inline constexpr std::string_view sbmv_name<r32, r32, r32> = "SSBMV ";
template<> inline constexpr std::string_view sbmv_name<r64, r64, r64> = "DSBMV ";
template<> inline constexpr std::string_view sbmv_name<c32, c32, c32> = "CSBMV ";
template<> inline constexpr std::string_view sbmv_name<c64, c64, c64> = "ZSBMV ";

template<typename IntType, typename AType, typename XType, typename YType>
PERFLIBS_LINALG_INLINE
bool sbmv_param_check(
...
void sbmv(
	...
	if constexpr(ParamCheck) {
		const auto res = perflibs::linalg::sbmv_param_check(
			uplo, n, k, alpha, a, lda, x, incx, beta, y, incy,
				sbmv_name<AType, XType, YType>);
	...
	spec::problem_context pctx {
		matmul::matmul3 {
			symmetric_matrix { a_uplo, banded_matrix_base { a, cntg  , a_strd, a_cntg_stride, a_strd_stride, kl, ku } },
...
#endif //PERFLIBS_LINALG_BLAS_INTERFACES_SBMV_HPP
