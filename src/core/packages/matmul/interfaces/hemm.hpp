...
#ifndef PERFLIBS_LINALG_BLAS_INTERFACES_HEMM_HPP
#define PERFLIBS_LINALG_BLAS_INTERFACES_HEMM_HPP
...
inline constexpr std::string_view hemm_name = "?HEMM ";

template<> inline constexpr std::string_view hemm_name<c32, c32, c32> = "CHEMM ";
template<> inline constexpr std::string_view hemm_name<c64, c64, c64> = "ZHEMM ";

template<typename IntType, typename AType, typename BType, typename CType>
static bool hemm_param_check(
...
void hemm(
	...
	if constexpr(ParamCheck) {
		const bool res = hemm_param_check(side, uplo, m, n, alpha, a, lda, b, ldb, beta, c, ldc,
			hemm_name<AType, BType, CType>);
		if(! res) return;
	}
	...
	if (option_matches(*side, 'L')) {
		...
		spec::problem_context pctx {
			matmul::matmul3 {
				...
				hermitian_matrix { b_uplo, matrix_base { b_ptr, cntg,   b_strd, b_cntg_stride, b_strd_stride } },
				general_matrix   {         matrix_base { c_ptr, a_strd, b_strd, c_cntg_stride, c_strd_stride } },
				*alpha, *beta
			},
			ArchitectureSpec { machine::get_system_unsafe() }
...
#endif //PERFLIBS_LINALG_BLAS_INTERFACES_HEMM_HPP
