...
#ifndef PERFLIBS_LINALG_BLAS_INTERFACES_SDOT_HPP
#define PERFLIBS_LINALG_BLAS_INTERFACES_SDOT_HPP
...
template<bool ParamCheck, typename IntType, typename XType,  typename YType, typename ReturnType, typename ComputePrecision, typename ArchitectureSpec>
...
ReturnType sdot(
	...
	auto c = zero<ComputePrecision>;

	if(*n <= 0)
		return *b;

	using scalar_type = ComputePrecision;
	...
	const     kernel_inttype c_strd_stride = 0;
	...
	//casts are required for bf16
	return static_cast<ReturnType>( c + static_cast<ComputePrecision>( *b ) );
}

} //namespace perflibs::linalg

#endif //PERFLIBS_LINALG_BLAS_INTERFACES_SDOT_HPP
