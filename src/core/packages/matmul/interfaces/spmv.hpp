...
inline constexpr std::string_view spmv_name = "?SPMV ";

template<> inline constexpr std::string_view spmv_name<r32, r32, r32> = "SSPMV ";
template<> inline constexpr std::string_view spmv_name<r64, r64, r64> = "DSPMV ";
template<> inline constexpr std::string_view spmv_name<c32, c32, c32> = "CSPMV ";
template<> inline constexpr std::string_view spmv_name<c64, c64, c64> = "ZSPMV ";

template<typename IntType, typename AType, typename XType, typename YType>
PERFLIBS_LINALG_INLINE
bool spmv_param_check(
...
void spmv(
	...
	if constexpr(ParamCheck) {
		const auto res = perflibs::linalg::spmv_param_check(
			uplo, n, alpha, ap, x, incx, beta, y, incy,
			spmv_name<AType, XType, YType>);
	...
	spec::problem_context pctx {
		matmul::matmul3 {
			symmetric_matrix { a_uplo, packed_matrix_base { ap, cntg  , a_strd, lower_flip(a_uplo)                   } },
...
#endif //PERFLIBS_LINALG_BLAS_INTERFACES_SPMV_HPP
