...
#ifndef PERFLIBS_LINALG_FACTORIZATION_STRATEGIES_APPLY_Q_FROM_QR_UNBLOCKED_HPP
#define PERFLIBS_LINALG_FACTORIZATION_STRATEGIES_APPLY_Q_FROM_QR_UNBLOCKED_HPP
...
class apply_q_from_qr_unblocked {
...
public:
static constexpr std::string_view name() { return "apply_q_from_qr_unblocked"; }
		...
		for (kernel_inttype i = start_idx; increment > 0 ? i <= end_idx : i >= end_idx; i += increment) {
			...
			auto larf_pctx = spec::problem_context{
				apply_elementary_reflector{
					pctx.side,
					to_const(pctx.a.sub_matrix(i, left ? nb_rows_left : nb_cols_left, i, 1)),
...
#endif // PERFLIBS_LINALG_FACTORIZATION_STRATEGIES_APPLY_Q_FROM_QR_UNBLOCKED_HPP
