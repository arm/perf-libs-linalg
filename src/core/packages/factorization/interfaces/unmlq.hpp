...
#ifndef PERFLIBS_LINALG_FACTORIZATION_INTERFACES_UNMLQ_HPP
#define PERFLIBS_LINALG_FACTORIZATION_INTERFACES_UNMLQ_HPP
...
IntType unmlq_min_lwork(const bool left, IntType m, IntType n) {
	if (m > 0 && n > 0) {
		return left ? n : m;
	}
	return 1;
...
PERFLIBS_LINALG_INLINE
bool unmlq_param_check(const char* side, const char* trans, const IntType* m, const IntType* n,
	...
	*info = 0;
	...
	IntType min_lwork = unmlq_min_lwork(left, *m, *n);
...
PERFLIBS_LINALG_INLINE
void unmlq(const char* side, const char* trans, const IntType* m, const IntType* n,
		const IntType* k, AType* a, const IntType* lda, AType* tau,
		AType* c, const IntType* ldc, AType* work, const IntType* lwork, IntType* info) {
	if constexpr (Paramcheck) {
		const bool res = unmlq_param_check(side, trans, m, n, k, a, lda, tau, c, ldc, work, lwork, info);
		if (!res) return;
	}
	...
	IntType opt_lwork  = unmlq_min_lwork(left, *m, *n);
	if (*m > 0 && *n > 0) {
	...
	// Matrix A dimensions - note: for LQ, A is k x (left ? m : n)
	const     kernel_inttype a_cntg      = *k;
	const     kernel_inttype a_strd      = left ? *m : *n;
...
#endif // PERFLIBS_LINALG_FACTORIZATION_INTERFACES_UNMLQ_HPP
