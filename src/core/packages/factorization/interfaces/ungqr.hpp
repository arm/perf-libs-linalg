...
#ifndef PERFLIBS_LINALG_FACTORIZATION_INTERFACES_UNGQR_HPP
#define PERFLIBS_LINALG_FACTORIZATION_INTERFACES_UNGQR_HPP
...
bool ungqr_param_check(const IntType* m, const IntType* n, const IntType* k,
...
PERFLIBS_LINALG_INLINE
void ungqr(const IntType* m, const IntType* n, const IntType* k,
           AType* a, const IntType* lda, AType* tau,
           AType* work, const IntType* lwork, IntType* info) {

	if constexpr (Paramcheck) {
		const bool res = ungqr_param_check(m, n, k, a, lda, tau, work, lwork, info);
		if (!res) return;
	}
	...
	const IntType opt_lwork = max_nb * (*n);
	work[0] = opt_lwork;
	...
	auto pctx = spec::problem_context{
		factorization::generate_q_from_qr{
...
#endif // PERFLIBS_LINALG_FACTORIZATION_INTERFACES_UNGQR_HPP
