...
#ifndef PERFLIBS_LINALG_FRAMEWORK_LINALG_BLAS_TYPES_HPP
#define PERFLIBS_LINALG_FRAMEWORK_LINALG_BLAS_TYPES_HPP
...
// Compare a single option character against one or more expected values, ignoring ASCII case.
template<typename... Others>
constexpr bool option_matches(char option, char expected, Others... others) {
	const char option_upper = ascii_upper(option);
	if constexpr (sizeof...(others) == 0) {
		return option_upper == ascii_upper(expected);
	}
	else {
		return option_upper == ascii_upper(expected) || ((option_upper == ascii_upper(others)) || ...);
	}
}
...
#endif //PERFLIBS_LINALG_FRAMEWORK_LINALG_BLAS_TYPES_HPP
