...
#ifndef SGEMM_UNROLLED_NT_NKM_HPP
#define SGEMM_UNROLLED_NT_NKM_HPP
...
void unrolled_kernel_NT_nkm(const kernel_inttype m, const kernel_inttype n, const kernel_inttype k, const float alpha,
	...
	for (int_type in = 0; in < n - (unrolln - 1); in+=unrolln) {
		...
		for (int_type ik = 0; ik < k - (unrollk - 1); ik+=unrollk) {
			...
			const float32_t *b_ptr = &B[ik*ldb + in];
