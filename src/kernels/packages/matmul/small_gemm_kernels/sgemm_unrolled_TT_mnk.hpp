...
#ifndef SGEMM_UNROLLED_TT_MNK_HPP
#define SGEMM_UNROLLED_TT_MNK_HPP
...
void unrolled_kernel_TT_mnk(const kernel_inttype n, const kernel_inttype m, const kernel_inttype k, const float alpha,
			...
			if (beta == float(0.0)) {
				for (int iun = 0; iun < unrolln; iun++) {
					for (int ium = 0; ium < unrollm; ium++) {
						...
						vst1q_lane_f32(c_ptr + ldc*4*ium + iun, cr0[unrollm*iun + ium], 0);
						vst1q_lane_f32(c_ptr + ldc*(4*ium + 1) + iun, cr0[unrollm*iun + ium], 1);
						vst1q_lane_f32(c_ptr + ldc*(4*ium + 2) + iun, cr0[unrollm*iun + ium], 2);
						vst1q_lane_f32(c_ptr + ldc*(4*ium + 3) + iun, cr0[unrollm*iun + ium], 3);
					}
				}
			}
			else {
				for (int iun = 0; iun < unrolln; iun++) {
					for (int ium = 0; ium < unrollm; ium++) {
						cr[unrollm*iun + ium] = vld1q_lane_f32(c_ptr + ldc*4*ium + iun, cr[unrollm*iun + ium], 0);
						cr[unrollm*iun + ium] = vld1q_lane_f32(c_ptr + ldc*(4*ium + 1) + iun, cr[unrollm*iun + ium], 1);
						cr[unrollm*iun + ium] = vld1q_lane_f32(c_ptr + ldc*(4*ium + 2) + iun, cr[unrollm*iun + ium], 2);
						cr[unrollm*iun + ium] = vld1q_lane_f32(c_ptr + ldc*(4*ium + 3) + iun, cr[unrollm*iun + ium], 3);

						cr0[unrollm*iun + ium] = vmulq_lane_f32(cr0[unrollm*iun + ium], alpha_rs, 0);

						cr[unrollm*iun + ium] = vfmaq_lane_f32(cr0[unrollm*iun + ium], cr[unrollm*iun + ium], beta_rs, 0);

						vst1q_lane_f32(c_ptr + ldc*4*ium + iun, cr[unrollm*iun + ium], 0);
						vst1q_lane_f32(c_ptr + ldc*(4*ium + 1) + iun, cr[unrollm*iun + ium], 1);
						vst1q_lane_f32(c_ptr + ldc*(4*ium + 2) + iun, cr[unrollm*iun + ium], 2);
						vst1q_lane_f32(c_ptr + ldc*(4*ium + 3) + iun, cr[unrollm*iun + ium], 3);
					}
				}
			}
		...
		for (int_type im = m - (m%4); im < m; im++) {
			...
			if (beta == float(0.0)) {
				for (int iun =0; iun < unrolln; iun++) {
					...
					vst1_lane_f32(c_ptr + iun, crs0[iun], 0);
				}
			}
			else {
				for (int iun =0; iun < unrolln; iun++) {
					crs[iun] = vld1_lane_f32(c_ptr + iun, crs[iun], 0);

					crs0[iun] = vmul_f32(crs0[iun], alpha_rs);

					crs[iun] = vfma_f32(crs0[iun], crs[iun], beta_rs);

					vst1_lane_f32(c_ptr + iun, crs[iun], 0);
				}
			}
		}
