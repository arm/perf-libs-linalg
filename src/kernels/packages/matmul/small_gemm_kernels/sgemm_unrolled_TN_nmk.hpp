...
#ifndef SGEMM_UNROLLED_TN_NMK_HPP
#define SGEMM_UNROLLED_TN_NMK_HPP
...
void unrolled_kernel_TN_nmk(const kernel_inttype m, const kernel_inttype n, const kernel_inttype k, const float alpha,
	...
	for (int in=0; in<n - (unrolln - 1); in+=unrolln) {
		...
		for (int im=0; im<m - (4*unrollm - 1); im+=4*unrollm) {
				...
				for (int ium=0; ium<unrollm; ium++) {
					...
					for (int iun=0; iun<unrolln; iun++) {
						for (int iuk=0; iuk<unrollk; iuk++) {
							...
							cr0[iun*unrollm + ium] = vfmaq_lane_f32(cr0[iun*unrollm + ium], ar[ium*unrollk + iuk], brs[iun*unrollk + iuk], 0);
						}
					}
				}
			...
			if (alpha == float(1.0) && beta == float(1.0)) {
				for (int iun=0; iun<unrolln; iun++) {
					for (int ium=0; ium<unrollm; ium++) {
						cr[iun*unrollm + ium] = vld1q_f32(c_ptr + iun*ldc + 4*ium);

						cr[iun*unrollm + ium] = vaddq_f32(cr0[iun*unrollm + ium], cr[iun*unrollm + ium]);

						vst1q_f32(c_ptr + iun*ldc + 4*ium, cr[iun*unrollm + ium]);
					}
				}
			}
			else if (alpha != float(1.0) && beta == float(1.0)) {
				for (int iun=0; iun<unrolln; iun++) {
					for (int ium=0; ium<unrollm; ium++) {
						cr[iun*unrollm + ium] = vld1q_f32(c_ptr + iun*ldc + 4*ium);

						cr0[iun*unrollm + ium] = vmulq_lane_f32(cr0[iun*unrollm + ium], alpha_rs, 0);
			...
			else if (alpha == float(1.0) && beta == float(0.0)) {
				for (int iun=0; iun<unrolln; iun++) {
					for (int ium=0; ium<unrollm; ium++) {
						vst1q_f32(c_ptr + iun*ldc + 4*ium, cr0[iun*unrollm + ium]);
					}
				}
			}
			else if (alpha != float(1.0) && beta == float(0.0)) {
			...
			else if (alpha == float(1.0) && beta != float(0.0) && beta!= float(1.0)) {
				for (int iun=0; iun<unrolln; iun++) {
					for (int ium=0; ium<unrollm; ium++) {
						cr[iun*unrollm + ium] = vld1q_f32(c_ptr + iun*ldc + 4*ium);

						cr[iun*unrollm + ium] = vfmaq_lane_f32(cr0[iun*unrollm + ium], cr[iun*unrollm + ium], beta_rs, 0);
		...
		for (int_type im = m - (m%4); im < m; im++) {
			...
			if (alpha == float(1.0) && beta == float(1.0)) {
				for (int iun=0; iun<unrolln; iun++) {
					crs[iun] = vld1_lane_f32(c_ptr + iun*ldc, crs[iun], 0);

					crs[iun] = vadd_f32(crs0[iun], crs[iun]);

					vst1_lane_f32(c_ptr + iun*ldc, crs[iun], 0);
				}
			}
			else if (alpha != float(1.0) && beta == float(1.0)) {
				for (int iun=0; iun<unrolln; iun++) {
