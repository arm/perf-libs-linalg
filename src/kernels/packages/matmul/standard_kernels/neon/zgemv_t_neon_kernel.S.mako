...
<%def name="zgemv(t)">
	...
	// We use x2 and x3 which originally store alpha as scratch after we already move them to FP registers.
	TMP1           .req x2
	TMP2           .req x3
	...
	LDA            .req x5
	X              .req x6
	// x7 stores incx originally, but we do not need it here. Use it for Y instead.
	Y              .req x7
	%else:
	...
	%if target_os in ('windows', 'windows_arm64ec'):
	INTER_COLUMN_STEP_TAIL .req x5 // The step needed to get from the end of one column to the start of the next in the tail loop
	%else:
	...
	// multiple times.
	%if target_os in ('windows', 'windows_arm64ec'):
	A_ADDR_0 .req x4 // A_ADDR_0 aliases A
	%else:
	...
	<% func_name = "zgemv_" + t + "_neon_kernel" %>
	${prologue(func_name)}
	stp x29, x30, [sp, #-160]!
	mov x29, sp
	stp  d8,  d9, [sp, #16]
		...
		%for is_tail, tail in [(False, ''), (True, 'tail')]:
			...
			<%def name="loop_unrolled(level, limit, size, is_tail, tail)">
				...
				%for l in range(level):
					ld2 { X_REG_X_V.2d, X_REG_Y_V.2d }, [X_ADDR], #32
			...
			%for column in range (NUM_COLUMNS):
				...
				// Store in Y
				ldr X_REG_X_D, [Y, #${16 * column}]
				ldr X_REG_Y_D, [Y, #${16 * column + 8}]
				...
				str X_REG_X_D, [Y, #${16 * column}]
				str X_REG_Y_D, [Y, #${16 * column + 8}]
				## Increment A_ADDR_X
				%if is_tail:
