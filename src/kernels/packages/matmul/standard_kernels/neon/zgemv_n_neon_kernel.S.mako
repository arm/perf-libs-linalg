...
LDA                    .req x5
X                      .req x6
// We can use x2 and x3 to store Y and as a scratch after moving alpha to FP registers
Y                      .req x3
TMP                    .req x2
%else:
M                      .req x0
...
fmov d0, x2
fmov d1, x3
mov ALPHA.d[0], v0.d[0]
mov ALPHA.d[1], v1.d[0]
// load beta from stack into v1
ldr q1, [sp, #160]
		...
		<%def name="loop_unrolled(level, limit, is_tail, size)">
			%if is_tail:
				...
				%for l in range(level):
					...
					ld2 {Y_REG_X_0_V.2d, Y_REG_Y_0_V.2d}, [Y_ADDR]
					...
					st2 {Y_REG_X_0_V.2d, Y_REG_Y_0_V.2d}, [Y_ADDR], #32
				%endfor
			...
			%else:
				...
				// Load all the Ys
				%for l in range(level):
					ld2 {Y_REG_X_${l}_V.2d, Y_REG_Y_${l}_V.2d}, [Y_ADDR], #32
				%endfor
				...
				// Store all Ys
				%for l in range(level):
					st2 {Y_REG_X_${l}_V.2d, Y_REG_Y_${l}_V.2d}, [Y_ADDR], #32
				%endfor
