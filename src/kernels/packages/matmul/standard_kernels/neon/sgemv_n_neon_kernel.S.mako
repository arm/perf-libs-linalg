...
dup ALPHA.4s, ALPHA.s[0]
dup BETA.4s, BETA.s[0]

// Setup the column increment
sub INTER_COLUMN_STEP_TAIL, LDA, M
lsl INTER_COLUMN_STEP_TAIL, INTER_COLUMN_STEP_TAIL, #2
...
%for unroll in UNROLL_PROGRESSION:
	...
	lsl x5, x5, #2 // Mul by 4 because floats are 4b
...
%for size in ["small", "big"]:
	...
	%for is_tail in [False, True]:
		...
		%else:
			%for c in range(COLUMNS_PER_LOOP):
				ldr X_REG_${c}_S, [X, ${c * 4}]
				dup X_REG_${c}_V.4s, X_REG_${c}_V.s[0]
			%endfor
		%endif
		mov Y_ADDR, Y
			...
			%else:
				...
				%for l in range(level):
					...
					%for c in range(COLUMNS_PER_LOOP):
						...
						%endif
						fmla Y_REG_${l}_V.4s, A_REG_${c}_V.4s, X_REG_${c}_V.4s
		...
		%else:
			b .Lrow_loop_tail_cond_${tail}_${size}
			.Lrow_loop_tail_${tail}_${size}:
			ldr TAIL_ACC_S, [Y_ADDR]
			%for c in range(COLUMNS_PER_LOOP):
				ldr A_REG_${c}_S, [A_ADDR_${c}]
				fmla TAIL_ACC_S, A_REG_${c}_S, X_REG_${c}_V.s[0]
			%endfor
			str TAIL_ACC_S, [Y_ADDR]
			%for c in range(COLUMNS_PER_LOOP):
				add A_ADDR_${c}, A_ADDR_${c}, #4
			%endfor
			add Y_ADDR, Y_ADDR, #4
			.Lrow_loop_tail_cond_${tail}_${size}:
			cmp Y_ADDR, M
