...
%endfor
// INC2: Allocate 2 extra y reg, for ld2 and st2
%for l in range(max(UNROLL_PROGRESSION)+1):
...
TAIL_ACC_V .req v${COLUMNS_PER_LOOP*2+3}
TAIL_ACC_Q .req q${COLUMNS_PER_LOOP*2+3}
TAIL_ACC_D .req d${COLUMNS_PER_LOOP*2+3}

<% func_name = "dgemv_inc2_n_neon_kernel" %>
${prologue(func_name)}
stp x29, x30, [sp, #-160]!
mov x29, sp
stp  d8,  d9, [sp, #16]
...
// INC2: Set this up lsl 4 rather than lsl 3, since the y vector is twice as
// long
lsl M, M, #4
add M, M, Y
...
%for size in ["small", "big"]:
	...
	%for is_tail in [False, True]:
		...
		%if is_tail:
			...
			dup v2.2d, v2.d[0]
		%else:
			// INC2: Load by 16s here since this is inc2, rather than 8
			%for c in range(COLUMNS_PER_LOOP):
				ldr X_REG_${c}_D, [X, ${c * 16}]
		...
		<%def name="loop_unrolled(level, limit, is_tail, size)">
			%if is_tail:
				...
				%for l in range(level):
					...
					// INC2: Ld2 here since incy = 2, only multiply v4 by beta
					ld2 {v4.2d, v5.2d}, [Y_ADDR]
					fmla v4.2d, v3.2d, v2.2d
					// INC2: Since v5 contains the 'other' untouched part of y, we can st2 here
					st2 {v4.2d, v5.2d}, [Y_ADDR], #32
				%endfor
			...
			%else:
				...
				// INC2: We no longer pre-load Y registers here, because ld2 has
				// no immediate addressing and is post-increment only.
				%for l in range(level):
					// INC2: LD2 here
					ld2 {Y_REG_${l}_V.2d, Y_REG_${l+1}_V.2d}, [Y_ADDR]
					...
					%for c in range(COLUMNS_PER_LOOP):
						...
						%endif
					%endfor
					// INC2: Store into Y with st2
					st2 {Y_REG_${l}_V.2d, Y_REG_${l+1}_V.2d}, [Y_ADDR], #32
				%endfor
				%for c in range(COLUMNS_PER_LOOP):
		...
		%if is_tail:
			add COLUMN, COLUMN, #1
			// INC2: Add by 16 since inc2
			add X, X, #16
			add A_ADDR_0, A_ADDR_0, INTER_COLUMN_STEP_TAIL
		%else:
			add COLUMN, COLUMN, #${COLUMNS_PER_LOOP}
			// INC2: Add by 16 * unroll since inc2
			add X, X, #${16 * COLUMNS_PER_LOOP}
			%for c in range(COLUMNS_PER_LOOP):
