...
<%def name="dgemv(is_parallel=False)">
	...
	%for column in range(NUM_COLUMNS):
		...
		ACC_REG_${column}_D .req d${3 + NUM_PRELOAD + column}
	%endfor
	%for pl in range(NUM_PRELOAD):
		A_REG_${pl}_V .req v${3 + pl}
		A_REG_${pl}_Q .req q${3 + pl}
		A_REG_${pl}_D .req d${3 + pl}
	%endfor

	%if is_parallel:
		<% func_name = "dgemv_t_neon_kernel_parallel" %>
	%else:
		<% func_name = "dgemv_t_neon_kernel_serial" %>
	%endif
	${prologue(func_name)}
	stp x29, x30, [sp, #-160]!
	mov x29, sp
