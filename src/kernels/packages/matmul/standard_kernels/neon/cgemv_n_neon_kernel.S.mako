...
// These kernels are used only when incx and incy is 1.
// We can discard incx and use the register as TMP.
// For Windows, alpha and beta are passed in via GPRs. So, we need to shift
// register numbers accordingly.
%if target_os in ('windows', 'windows_arm64ec'):
	...
	LDA                    .req x4
	X                      .req x5
	// We can continue placing Y at x6 which originally is for INCX
	Y                      .req x6
	// use register for ALPHA as scratch because we already move it to FP register
	TMP                    .req x2
%else:
	M                      .req x0
	N                      .req x1
	...
	X                      .req x4
	Y                      .req x6
	// use register for INCX as scratch
	TMP                    .req x5
%endif
ALPHA                  .req v0
BETA                   .req v1
...
%endfor
## Avoid using x13 and x14 so code works on Arm64EC
N_ROUND_DOWN           .req x15
INTER_COLUMN_STEP      .req x16
INTER_COLUMN_STEP_TAIL .req x17
...
%for l in range(max(UNROLL_PROGRESSION)):
	...
	Y_REG_X_${l}_S .req s${COLUMNS_PER_LOOP*3+2+l*2}
	...
	Y_REG_Y_${l}_S .req s${COLUMNS_PER_LOOP*3+3+l*2}
%endfor
...
mov TMP, #${COLUMNS_PER_LOOP-1}
madd INTER_COLUMN_STEP, LDA, TMP, INTER_COLUMN_STEP
lsl INTER_COLUMN_STEP, INTER_COLUMN_STEP, #3

// Setup the rounded down versions of M and N
%for unroll in UNROLL_PROGRESSION:
	mov TMP, #${unroll * 8}
	udiv  M_ROUND_DOWN_${unroll}, M, TMP
	lsl TMP, TMP, #3
	mul  M_ROUND_DOWN_${unroll}, M_ROUND_DOWN_${unroll}, TMP
%endfor
mov TMP, #${COLUMNS_PER_LOOP}
udiv N_ROUND_DOWN,   N, TMP
mul  N_ROUND_DOWN,   N_ROUND_DOWN,   TMP
...
%for size in ["small", "big"]:
	...
	%for is_tail in [False, True]:
		...
		<%def name="loop_unrolled(level, limit, is_tail, size)">
			%if is_tail:
				...
				%for l in range(level):
					...
					ld2 {Y_REG_X_0_V.4s, Y_REG_Y_0_V.4s}, [Y_ADDR]
					fmla Y_REG_X_0_V.4s, A_REG_X_0_V.4s, X_REG_0_V.s[0]
					fmls Y_REG_X_0_V.4s, A_REG_Y_0_V.4s, X_REG_0_V.s[1]
					fmla Y_REG_Y_0_V.4s, A_REG_X_0_V.4s, X_REG_0_V.s[1]
					fmla Y_REG_Y_0_V.4s, A_REG_Y_0_V.4s, X_REG_0_V.s[0]
					st2 {Y_REG_X_0_V.4s, Y_REG_Y_0_V.4s}, [Y_ADDR], #32
				%endfor
			...
			%else:
				...
				// Load all the Ys
				%for l in range(level):
					ld2 {Y_REG_X_${l}_V.4s, Y_REG_Y_${l}_V.4s}, [Y_ADDR], #32
				%endfor
				// Reset Y for storing w/ post-index addressing
				sub Y_ADDR, Y_ADDR, ${level * 32}
				%for l in range(level):
					// Load from A and fmla into Y
					## Preload A reg
					ld2 {A_REG_X_0_V.4s, A_REG_Y_0_V.4s}, [A_ADDR_0], #32
					% if COLUMNS_PER_LOOP > 1:
						ld2 {A_REG_X_1_V.4s, A_REG_Y_1_V.4s}, [A_ADDR_1], #32
					%endif
					%for c in range(COLUMNS_PER_LOOP):
						%if c < COLUMNS_PER_LOOP-2:
							ld2 {A_REG_X_${c+2}_V.4s, A_REG_Y_${c+2}_V.4s}, [A_ADDR_${c+2}], #32
						%endif
						fmla Y_REG_X_${l}_V.4s, A_REG_X_${c}_V.4s, X_REG_${c}_V.s[0]
						fmls Y_REG_X_${l}_V.4s, A_REG_Y_${c}_V.4s, X_REG_${c}_V.s[1]
						fmla Y_REG_Y_${l}_V.4s, A_REG_X_${c}_V.4s, X_REG_${c}_V.s[1]
						fmla Y_REG_Y_${l}_V.4s, A_REG_Y_${c}_V.4s, X_REG_${c}_V.s[0]
				...
				// Store all Ys
				%for l in range(level):
					st2 {Y_REG_X_${l}_V.4s, Y_REG_Y_${l}_V.4s}, [Y_ADDR], #32
				%endfor
		...
		%if is_tail:
			...
			ldr A_REG_X_0_S, [A_ADDR_0]
			ldr A_REG_Y_0_S, [A_ADDR_0, #4]
			ldr Y_REG_X_0_S, [Y_ADDR]
			ldr Y_REG_Y_0_S, [Y_ADDR, #4]
			fmla Y_REG_X_0_S, A_REG_X_0_S, X_REG_0_V.s[0]
			fmls Y_REG_X_0_S, A_REG_Y_0_S, X_REG_0_V.s[1]
			fmla Y_REG_Y_0_S, A_REG_X_0_S, X_REG_0_V.s[1]
			fmla Y_REG_Y_0_S, A_REG_Y_0_S, X_REG_0_V.s[0]
			str Y_REG_X_0_S, [Y_ADDR]
			str Y_REG_Y_0_S, [Y_ADDR, #4]
		...
		%else:
			b .Lrow_loop_tail_cond_${tail}_${size}
			.Lrow_loop_tail_${tail}_${size}:
			ldr TAIL_ACC_X_S, [Y_ADDR]
			ldr TAIL_ACC_Y_S, [Y_ADDR, #4]
			%for c in range(COLUMNS_PER_LOOP):
				ldr A_REG_X_${c}_S, [A_ADDR_${c}]
				ldr A_REG_Y_${c}_S, [A_ADDR_${c}, #4]
				fmla TAIL_ACC_X_S, A_REG_X_${c}_S, X_REG_${c}_V.s[0]
				fmls TAIL_ACC_X_S, A_REG_Y_${c}_S, X_REG_${c}_V.s[1]
				fmla TAIL_ACC_Y_S, A_REG_X_${c}_S, X_REG_${c}_V.s[1]
				fmla TAIL_ACC_Y_S, A_REG_Y_${c}_S, X_REG_${c}_V.s[0]
			%endfor
			str TAIL_ACC_X_S, [Y_ADDR]
			str TAIL_ACC_Y_S, [Y_ADDR, #4]
			%for c in range(COLUMNS_PER_LOOP):
