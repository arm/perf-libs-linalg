...
#define min s0
#define temp1 s1    //General temporary variable storing current element
#define temp2 s2    //Used to iterate through the vector in L64 reduction

	<% func_name = "isamin_kernel" %>
	${prologue(func_name)}
	stp x29, x30, [sp, #-16]!
	mov x29, sp
...
.Loop64_check:
	asr iterations, N, 6    //compute the number of iterations for Loop64
	cmp iterations, xzr
	beq .Lscalar_check
	add Z, Z, 1
...
.Loop64:
	ldp q2, q3, [X]
	ldp q4, q5, [X, 8*ELEM_SIZE]
	ldp q6, q7, [X, 16*ELEM_SIZE]
	ldp q16, q17, [X, 24*ELEM_SIZE]
	ldp q18, q19, [X, 32*ELEM_SIZE]
	ldp q20, q21, [X, 40*ELEM_SIZE]
	ldp q22, q23, [X, 48*ELEM_SIZE]
	ldp q24, q25, [X, 56*ELEM_SIZE]
	...
	fmin v18.4s, v18.4s, v19.4s
	fmin v20.4s, v20.4s, v21.4s
	fmin v22.4s, v22.4s, v23.4s
	fmin v24.4s, v24.4s, v25.4s
